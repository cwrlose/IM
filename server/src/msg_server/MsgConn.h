...
private:
    void _HandleHeartBeat(CImPdu* pPdu);//发送心跳包
	void _HandleLoginRequest(CImPdu* pPdu);//客户端发起登录请求，需要与 DBServer 交互进行身份验证
    void _HandleLoginOutRequest(CImPdu* pPdu);//下线
    void _HandleClientRecentContactSessionRequest(CImPdu* pPdu);
	void _HandleClientMsgData(CImPdu* pPdu);//处理客户端发送的聊天消息（核心消息转发）。
	void _HandleClientMsgDataAck(CImPdu* pPdu);//处理客户端发送的聊天消息（核心消息转发）。
	void _HandleClientTimeRequest(CImPdu* pPdu);
    void _HandleClientGetMsgListRequest(CImPdu* pPdu);//客户端请求获取指定会话的历史消息列表。
    void _HandleClientGetMsgByMsgIdRequest(CImPdu* pPdu);//客户端请求获取指定 Msg ID 的消息内容。
	void _HandleClientUnreadMsgCntRequest(CImPdu* pPdu);//客户端请求获取未读消息总数或按会话的未读数。
	void _HandleClientMsgReadAck(CImPdu* pPdu);//客户端对已读消息的确认回复，通知服务器更新已读标记。
    void _HandleClientGetLatestMsgIDReq(CImPdu* pPdu);//客户端对已读消息的确认回复，通知服务器更新已读标记。
