...
puts("db init success");
	// 主线程初始化单例，不然在工作线程可能会出现多次初始化 //数据库操作
    ...
    char* str_file_site = config_file.GetConfigName("MsfsSite");//图片服务器
    char* str_aes_key = config_file.GetConfigName("aesKey");//加密
